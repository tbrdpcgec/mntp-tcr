...
import W302PDF from '../components/W302PDF';
...
function DownloadPDFButton({
  ...
  const handleConfirm = async () => {
    ...
    link.href = url;
    link.download = `E-HOB MANTAP W302 ${formattedDate} ${shift}.pdf`;
...
const managerOptions = ['532889 / PURWANTO'];
...
const columnWidths: Record<string, string> = {
  ...
  remark: 'min-w-[150px]',
  status_cs1: 'min-w-[90px]',
  remark_cs1: 'min-w-[250px]',
  handle_by_cs1: 'min-w-[90px]',
  date_closed_cs1: 'min-w-[80px]',
  report_cs1: 'min-w-[0px]',
  archive_cs1: 'min-w-[50px]',
};

const COLUMN_ORDER = [
  { key: 'no', label: 'No' },
  { key: 'report_cs1', label: 'Report' },
  ...
  { key: 'archive_cs1', label: 'Archived' },
];
...
const HANDLE_BY_OPTIONS: string[] = [
  'PION',
  'ERWIN',
  'WAGINO',
  'SETIAJI',
...
export default function W302() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // 🔴 FILTER UTAMA TCR-1
      let filtered = allRows.filter((r) => r.cek_cs1 === 'red');

      // 🔴 archive_sm1 filter pakai toggle
      if (!showArchivedSM1) {
        filtered = filtered.filter(
          (r) => r.archive_cs1 === '' || r.archive_cs1 === null
        );
      }
  ...
  const handleUpdate = async (id: string, key: string, value: any) => {
    ...
    if (key === 'status_cs1' && value === 'CLOSED') {
      updates['date_closed_cs1'] = formatDateToDDMMMYYYY(new Date());
    }
              ...
              onClick={() => {
                ...
                const selectedData = rows
                  ...
                  .map((row) => [
                    ...
                    clean(row.handle_by_cs1),
                    clean(row.status_cs1),
                    clean(row.remark_cs1),
                  ])
                ...
                >
                  {COLUMN_ORDER.map(({ key }) => (
                    ...
                    >
                      ...
                      ) : key === 'date_in' || key === 'date_closed_cs1' ? (
                        row[key] ? (
